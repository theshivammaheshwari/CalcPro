...
export default function SwingTrading() {
  ...
  const handleAddPortfolio = async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/stock/quotes?symbols=${encodeURIComponent(symbolToUse)}`);
      if (res.ok) {
         const data = await res.json();
         setWishlistQuotes(prev => ({...prev, ...data}));
      }
                             ...
                             <div className="text-right">
                               {currentPrice > 0 ? (
                                  <>
                                     ...
                                     <p className={`text-xs font-bold flex items-center justify-end gap-0.5 ${isUp ? 'text-emerald-600' : 'text-rose-600'}`}>
                           ...
                           {hasPosition ? (
                              <div className="mt-4 pt-4 border-t border-gray-100 grid grid-cols-2 gap-4">
                                ...
                                <div className="text-right">
                                  ...
                                  <p className={`font-mono font-bold text-sm ${pnlIsUp ? 'text-emerald-600' : 'text-rose-600'}`}>
                                     {pnlIsUp ? '+' : ''}₹{formatVal(pnl)}
                                  </p>
                                  <p className={`text-xs font-bold ${pnlIsUp ? 'text-emerald-600' : 'text-rose-600'}`}>
