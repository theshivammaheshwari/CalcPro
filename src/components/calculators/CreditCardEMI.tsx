...
export default function CreditCardEMI() {
  ...
  const calculate = () => {
    ...
    const emiExGst = (principal * monthlyRate * Math.pow(1 + monthlyRate, tenureMonths)) / (Math.pow(1 + monthlyRate, tenureMonths) - 1);
    const processingFeeAmount = (principal * processingFeeRate) / 100;
    const gstOnProcessingFee = (processingFeeAmount * gstRate) / 100;
    const schedule: AmortizationRow[] = [];
    let balance = principal, totalInterest = 0, totalGst = 0;
    for (let month = 1; month <= tenureMonths; month++) {
      ...
      schedule.push({ month, openingBalance: balance, emiExGst, interest, principal: principalPayment, gstOnInterest, emiWithGst, closingBalance: balance - principalPayment });
      balance -= principalPayment;
      totalInterest += interest;
      totalGst += gstOnInterest;
    }
    const totalPaid = (emiExGst * tenureMonths) + totalGst + processingFeeAmount + gstOnProcessingFee;
    setResults({ monthlyEmi: emiExGst, processingFeeAmount, gstOnProcessingFee, totalInterest, totalGst, totalPaid, amortizationSchedule: schedule });
    setAnimKey(k => k + 1);
  };
      ...
      <div className="grid lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-1">
          <div className="bg-blue-50 rounded-2xl p-6 space-y-5">
            <h3 className="font-semibold text-blue-800 flex items-center gap-2"><CreditCard className="w-5 h-5" /> Loan Details</h3>
            {[
              ...
              { label: 'GST on Interest (%)', field: 'gstRate', placeholder: '18', step: '0.1' },
              { label: 'Processing Fee (%)', field: 'processingFeeRate', placeholder: '1', step: '0.1' },
            ...
            <>
              <div className="grid sm:grid-cols-2 gap-4">
                <div className="result-card bg-gradient-to-br from-green-500 to-emerald-600 text-white">
                  ...
                  <p className="text-green-100 text-xs mt-1">EMI (ex-GST)</p>
                </div>
                <div className="result-card bg-gradient-to-br from-orange-500 to-amber-500 text-white">
                  ...
                  <p className="text-orange-100 text-xs mt-1">+ {fmt(results.gstOnProcessingFee)} GST</p>
                </div>
              </div>

              <div className="bg-white rounded-2xl p-6 border border-gray-100 shadow-sm">
                <h3 className="font-semibold text-gray-800 mb-4 flex items-center gap-2"><TrendingUp className="w-5 h-5 text-indigo-600" /> Payment Summary</h3>
                <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
                  {[
                    ...
                    { label: 'Total GST', value: results.totalGst, color: 'text-purple-700', bg: 'bg-purple-50' },
                    ...
                    { label: 'Extra Paid', value: results.totalPaid - inputs.principal, color: 'text-red-700', bg: 'bg-red-50' },
        ...
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
          ...
          <div className="overflow-x-auto">
            <table className="w-full calc-table">
              <thead>
                <tr>
                  {['Month', 'Opening', 'EMI (ex-GST)', 'Interest', 'Principal', 'GST', 'EMI+GST', 'Closing'].map(h => (
                    <th key={h} className="first:rounded-tl-none">{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                {results.amortizationSchedule.map((row, i) => (
                  <tr key={row.month} className={i % 2 === 0 ? 'bg-gray-50/50' : ''}>
                    ...
                    <td className="text-gray-600">{fmt(row.emiExGst)}</td>
                    <td className="text-red-600">{fmt(row.interest)}</td>
                    <td className="text-green-600">{fmt(row.principal)}</td>
                    <td className="text-orange-600">{fmt(row.gstOnInterest)}</td>
                    <td className="text-blue-600 font-medium">{fmt(row.emiWithGst)}</td>
                ...
                <tr className="bg-gray-100 font-bold border-t-2 border-gray-300">
                  ...
                  <td>{fmt(results.amortizationSchedule.reduce((s, r) => s + r.emiExGst, 0))}</td>
                  <td className="text-red-700">{fmt(results.totalInterest)}</td>
                  <td className="text-green-700">{fmt(inputs.principal)}</td>
                  <td className="text-orange-700">{fmt(results.totalGst)}</td>
                  <td className="text-blue-700">{fmt(results.amortizationSchedule.reduce((s, r) => s + r.emiWithGst, 0))}</td>
                  <td>₹0</td>
                </tr>
              </tbody>
            </table>
