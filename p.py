...
idx1 = text.find('{wishlist.length === 0 ? (')
idx2 = text.find('{/* View: Compare */}')

if idx1 == -1 or idx2 == -1:
    print("Not found", idx1, idx2)
else:
    new_text = r"""              {wishlist.length === 0 ? (
    ...
    with open('src/components/calculators/SwingTrading.tsx', 'w', encoding='utf-8') as f:
        f.write(text[:idx1] + new_text + text[idx2:])
    
    print("Done rendering html table!")
