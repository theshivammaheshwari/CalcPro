...
var idx = c.indexOf('{wishlist.length === 0 ? (\\n                 <div className="text-center p-8 bg-gray-50 border border-gray-200 border-dashed rounded-xl">');
if (idx === -1) idx = c.indexOf('{wishlist.length === 0 ? (');
var idx2 = c.indexOf('{/* View: Compare */}');

if (idx !== -1 && idx2 !== -1) {
    fs.writeFileSync('src/components/calculators/SwingTrading.tsx', c.substring(0, idx) + tNew + c.substring(idx2));
    console.log('Force table replace success via node script!');
} else {
    console.log('Not found:', idx, idx2);
}
